...
POST_QUEUE = MessageQueue()  # 註冊一個推播的queue，每5秒推一則，server才不會爆掉
sched.add_job(post_service.post_to_line, 'interval', seconds=5, args=[POST_QUEUE])
ANSWER_QUEUE = MessageQueue()  # 註冊一個回答的queue，每5秒推一則，server才不會爆掉
sched.add_job(post_service.answer_to_poster, 'interval', seconds=5, args=[ANSWER_QUEUE])
sched.start()
